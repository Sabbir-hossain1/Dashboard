(function(global, factory) {
    ...
    console.log({
        params: params
    }), Object.keys(params).length > 0 && Object.keys(params).includes("theme-control") && Object.keys(CONFIG).forEach((a => {
        localStorage.setItem(a, CONFIG[a]);
    })), Object.keys(params).forEach((a => {
        console.log({
            param: a
        }), console.log(configQueryMap[a], params[a]), configQueryMap[a] && localStorage.setItem(configQueryMap[a], params[a]);
    })), Object.keys(CONFIG).forEach((a => {
        null === localStorage.getItem(a) && localStorage.setItem(a, CONFIG[a]);
